...
import io.github.contextawareness.utils.IOUtils;
...
import static io.github.contextawareness.utils.Assertions.cast;
...
public class Item {
    ...
    public void joinItem(Item input) {
        for (String key : input.itemMap.keySet()) {
            if (key != TIME_CREATED) {
                this.setFieldValue(key, input.itemMap.get(key));
            }
        }
        // Join 结束时，重新设定对象的创建时间
        this.setFieldValue(TIME_CREATED, System.currentTimeMillis());
    }
}
